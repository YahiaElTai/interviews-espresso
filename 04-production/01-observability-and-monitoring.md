...
- Log aggregation architecture — collectors/agents, pipelines (Fluentd/Fluent Bit/Alloy), centralized storage, retention policies, managed vs self-hosted tradeoffs (ELK, Loki, CloudWatch, Datadog)
- Observability cost and data management — log volume control, metric cardinality limits, trace sampling rates, retention tiers, balancing visibility against storage/ingestion costs
...
- Dashboard design principles — service-level overview vs drill-down, RED/USE layout, connecting dashboards to on-call runbooks for actionable incident response
- Health check endpoints (/health, /ready) — K8s probe integration, dependency checks
- Observability during deployments — deployment markers/annotations, canary metrics comparison, automated rollback triggers based on SLO violations
- Error tracking and reporting — unhandled exception capture, error grouping and deduplication, linking errors to deployments, Sentry as a practical example
...
<details>
<summary>12. Why do static threshold alerts fail for SLO-based reliability — how does burn-rate alerting work, what is a burn rate and how do multi-window burn rates reduce false positives while still catching real incidents fast enough?</summary>
...
<details>
<summary>13. How does a log aggregation pipeline work end to end — what are the roles of collectors/agents, processing pipelines, and storage backends, what are the retention and cost tradeoffs between different solutions, and how do you prevent log volume from spiraling into an expensive problem?</summary>
...
<details>
<summary>14. Why do production systems need dedicated error tracking (e.g., Sentry) beyond what logs and metrics provide — what does error tracking give you that grep on log files doesn't (grouping, deduplication, release tracking, stack traces with context), and how does linking errors to specific deployments change how you respond to regressions?</summary>
...
<details>
<summary>16. What makes a production dashboard useful vs decorative — how do you structure service-level overview dashboards vs drill-down dashboards, why should dashboards follow RED/USE layouts, and how do you connect dashboards to on-call runbooks so the person investigating an alert can immediately take action?</summary>
...
<details>
<summary>19. Instrument a Node.js service with OpenTelemetry — show the setup for auto-instrumentation (HTTP, database calls), add a manual span for a custom business operation, configure the collector pipeline (exporter, processor, batching), and explain what each piece does and what you lose without it</summary>
...
<details>
<summary>20. Implement correlation ID middleware for a Node.js HTTP service — show how to generate an ID for incoming requests (or extract one from upstream headers), propagate it through async context so all log lines include it, pass it to downstream HTTP/gRPC calls, and explain what breaks in cross-service debugging without this</summary>
...
<details>
<summary>22. Implement /health and /ready endpoints for a Node.js service that checks downstream dependencies (database, cache, external API) — show the code, explain the difference between liveness and readiness semantics, show the Kubernetes probe YAML that integrates with these endpoints, and explain what happens when readiness checks are too aggressive or too lenient</summary>
...
<details>
<summary>25. Design a log aggregation pipeline for a Kubernetes-based microservices application — show the collector/agent configuration, explain how logs flow from containers to the aggregation backend, define retention tiers (hot/warm/cold), and walk through the cost optimization decisions you'd make when log volume reaches terabytes per day</summary>
...
<details>
<summary>26. Set up observability for a deployment pipeline — show how to add deployment markers/annotations to your metrics dashboards so you can correlate changes in behavior with releases, explain how you compare canary vs baseline metrics during a rollout, and configure an automated rollback trigger based on SLO violation thresholds. Walk through what each piece catches that manual monitoring would miss</summary>
...
<details>
<summary>27. An API endpoint's p99 latency spiked from 200ms to 2s starting 30 minutes ago — walk through step by step how you use metrics to identify which endpoint and when, traces to find the slow span, and logs to find the root cause. Show the specific queries and reasoning at each step</summary>
...
<details>
<summary>30. Your OpenTelemetry traces have gaps — spans from Service A appear, but the child spans from Service B (which Service A calls) are disconnected and show up as separate root traces. Walk through how you diagnose and fix context propagation failures between services</summary>
...
<details>
<summary>31. Your on-call engineer is getting 50+ alerts per shift and most are non-actionable — they've started ignoring pages. Walk through how you audit the existing alerts, identify which ones to delete, consolidate, or restructure, and design an alerting hygiene process that prevents this from happening again</summary>
...
<details>
<summary>32. Set up Sentry error tracking in a Node.js service — show the integration code with request context enrichment (user, route, correlation ID), source map configuration for readable stack traces in production, and explain how Sentry's error grouping and deduplication reduce noise compared to searching raw logs for exceptions</summary>
...
<details>
<summary>33. Tell me about a time you set up or significantly improved observability for a production system — what was the state before, what did you implement, and how did it change the team's ability to detect and respond to issues?</summary>
...
<details>
<summary>34. Describe a time you used observability tooling (metrics, traces, logs) to diagnose a complex production incident that would have been very difficult to solve without it — what were the symptoms, how did you correlate signals, and what was the root cause?</summary>
...
<details>
<summary>35. Tell me about a time you dealt with alert fatigue or redesigned an alerting strategy — what was wrong with the existing alerts, what changes did you make, and how did you measure whether the new approach was better?</summary>
...
<details>
<summary>36. Describe a time you had to balance observability cost with coverage — what tradeoffs did you make around log retention, trace sampling, or metric cardinality, and how did you decide what was acceptable to lose?</summary>
